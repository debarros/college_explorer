...
shinyServer(function(input, output) {
  ...
  }) # /reactive defintion of data object
  ...
  observe({
    ...
    if(nrow(data()) != 0){   #some of the functions below break down if data() is empty so I need this if statement
      ...
      size <- sqrt(x / quantile(x, 0.95, na.rm = T) * 440) 
      
      leafletProxy("map", data = data()) %>%
        clearMarkers() %>%                    # you have to clear previously drawn markers
        addCircleMarkers(lng = ~lon, lat = ~lat, stroke = F, popup = ~popup, label = ~INSTNM, radius = size, fillOpacity = 0.5, fillColor = colors) %>%
        addLegend("bottomleft",  colors = colors_list, labels = values_list, layerId = "legend")
      
    } else {
      leafletProxy("map") %>% clearMarkers() # clear the map if the data() is empty
    } # /if-else
  ...
  # output$list <- renderDataTable(select(data(), linked_name, adm_rate, SATVR75), options = list(pageLength = 25, spacing = "xs"), escape = FALSE)
  
  # Unfolding UI ####
  # below is what is needed for the "unfolding" UI
  onclick("toggleprice",       toggle(id = "filterprice",       anim = T))
  onclick("toggleSAT",         toggle(id = "filterSAT",         anim = T))
  onclick("toggleoutcomes",    toggle(id = "filteroutcomes",    anim = T))
  onclick("togglefinoutcomes", toggle(id = "filterfinoutcomes", anim = T))
  onclick("togglesize",        toggle(id = "filtersize",        anim = T))
  onclick("toggleabout",       toggle(id = "about",             anim = T))
